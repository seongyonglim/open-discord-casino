...
export const SIDE = `    var paidSeat = {}, paidSeatHand = null;
    ...
    function renderPrizeTab(){
      ...
      prizeTabEl.innerHTML =
        ...
        (isPko
          ? '<div class="ht-pz-bty">' +
              /* 총 현상금 옆에 내가 벌어 둔 몫을 붙인다 — 전체 중 얼마를 챙겼는지가
                 한 줄에서 읽힌다. 따로 떨어뜨려 두면 두 숫자를 눈으로 비교해야 한다.
                 "확보"라고 적는다: 바운티는 대회가 끝날 때 한 번에 나가므로 아직 지갑에
                 없는 돈이다. "획득"이라고 적으면 잔액을 확인하러 갔다가 없어서 버그로
                 읽는다 — 아래 규칙 줄에도 언제 들어오는지를 함께 적어 둔다. */
              '<div class="ht-pz-bty-h">바운티 상금 <b>' + num(btyPool) + 'P<\\/b>'
                + (t.myBountyWon > 0
                  ? '<em>내가 확보 ' + num(t.myBountyWon) + 'P<\\/em>' : '') + '<\\/div>' +
              /* 내 봉투/머리 값은 따로 둔다 — 위의 "확보"는 이미 내 것으로 굳은 값이고
                 이쪽은 아직 남에게 사냥당할 수 있는 값이라 성격이 다르다.
              ...
              '<div class="ht-pz-bty-b">' + (mystery
                ? '<b>모두의 바운티 금액이 다르고, 잡히기 전까지 비공개입니다.<\\/b><br>'
                  + '· 상대를 탈락시키면 그 사람 봉투가 열리고 <b>전액이 내 몫이 됩니다<\\/b><br>'
                  + '· 내 봉투는 <b>우승하면 내가 받습니다<\\/b><br>'
                  /* 잡은 사람이 봉투를 독식하므로 내 머리 값은 오르지 않는다 —
                     PKO 와 다른 유일한 규칙이라 반드시 적어 준다. */
                  + (hasRank
                    ? '· 순위 상금은 등수로 따로 나눕니다<br>'
                    : '· 순위 상금은 없습니다 — 끝까지 버티면 본전, 잡으면 그만큼 법니다<br>')
                  + '· 확보한 바운티는 <b>대회가 끝날 때 한 번에<\\/b> 들어옵니다'
                : '· 상대를 탈락시키면 그 사람 바운티의 <b>절반이 내 몫이 됩니다<\\/b><br>'
                  + '· 남은 절반은 <b>내 바운티에 얹힙니다<\\/b><br>'
                  + '· 내 바운티는 <b>우승하면 전부 받습니다<\\/b><br>'
                  + '· 확보한 바운티는 <b>대회가 끝날 때 한 번에<\\/b> 들어옵니다')
              + '<\\/div>' +
            '<\\/div>'
          : '') +
