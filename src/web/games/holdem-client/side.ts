...
export const SIDE = `    var paidSeat = {}, paidSeatHand = null;
    ...
    function renderPrizeTab(){
      ...
      /* 바운티가 걸린 판인가 / 그중 미스터리인가. 미스터리는 순위 상금이 없어 갈래를
         나눠 적을 것이 없다 — 숫자 하나와 규칙 한 줄로 끝난다. */
      ...
      prizeTabEl.innerHTML =
        '<div class="ht-pz-head">' +
          ...
          /* 미스터리에는 등수로 주는 상금이 없으니 "지급 N명"이 거짓이 된다 */
          '<span>참가 ' + t.registered + '명'
            + (mystery ? ' · 순위 상금 없음' : ' · 지급 ' + t.itm + '명') + '<\\/span>' +
        '<\\/div>' +
        ...
        (isPko && !mystery
          ...
          : '') +
        /* 미스터리는 순위 상금이 없으니 등수 표를 그리지 않는다 — 0P 짜리 표는
           "1위 0P" 라고 적힌 거짓말이 된다. */
        (mystery ? '' : '<div class="ht-pz-list">' + prizeListHtml(t) + '<\\/div>') +
