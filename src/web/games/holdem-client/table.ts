...
export const TABLE = `    function renderTable(){
      ...
      noteHoleReveal(tb);
      syncBoard(tb);
      /* 팟 금액. 정산이 시작되면 층을 하나씩 보내면서 그만큼 줄어들고, 마지막 층에서
         정확히 0 이 된다(payLayer 가 potShown 을 관리한다). 그 값이 이 판의 것일 때는
         서버 값(tb.pot)이 아니라 그쪽을 그린다 — 폴링이 1초마다 이 함수를 부르므로,
         여기서 서버 값으로 되돌리면 줄어든 숫자가 곧바로 총액으로 되살아난다.

         한때는 정산 뒤에도 총액을 그대로 남겨 두었다("방금 얼마짜리 판이었나"를 볼 수
         있게). 그런데 칩이 이미 다 날아간 뒤에도 "Total Pot 3,000"이 떠 있으면 같은 돈이
         두 곳에 있는 것으로 보인다 — 그래서 0 으로 비우는 쪽으로 바꿨다. 판의 크기는
         승자 스택이 오르는 것과 WIN 배지로 읽힌다. */
      potEl.textContent = stackText(potShownHand === tb.handNo ? potShown : tb.pot);
      renderSeats();
      dealSequence(tb);
      renderSide();
