...
(function(){
  ...
  var SFX_SETS = {
    ...
    gunshot: ['gunshot'],
    /* 미스터리 전광판이 멈추는 소리 — 음원 파일이 아직 없다. 없으면 playSample 이 false 를
       돌려주고 합성음이 대신 울린다(우승 음원과 같은 방식이다). 파일을 넣는 순간 자동으로
       그쪽이 쓰인다 — SFX_NORM 에 보정값만 재서 넣으면 된다.
       요구 사양은 docs/audio-requests.md 에 적어 뒀다.

       굴러가는 소리(reelTick)는 여기 없다. 숫자가 바뀌는 간격에 딱 맞아야 하는 소리라
       파일로 두면 그 간격과 어긋난다 — 합성음만 쓴다. */
    reelstop: ['reel-stop'],
  ...
  window.casinoSfx = {
    ...
    /* ── 미스터리 바운티 전광판 ──────────────────────────────────────
       ...
       굴러가는 소리는 음원을 두지 않는다. 숫자가 바뀌는 간격(MYS_TICK_MS)에 딱 맞아야
       하는 소리라, 파일로 두면 그 간격과 어긋난다 — 눈은 55ms 마다 바뀌는데 소리는
       제 리듬으로 가면 "소리 따로 그림 따로"가 된다. 그래서 숫자가 바뀔 때마다
       한 번씩 부른다(reelTick). 멈추는 소리만 음원 슬롯을 둔다. */
    /* 릴이 한 칸 넘어가는 소리. 아주 짧고 아주 작아야 한다 — 1.6초에 서른 번쯤 울리므로
       조금만 커도 귀를 때리는 소음이 된다. 고역 딸깍 하나로 끝낸다. */
    reelTick: function(){
      var c = ac(); if (!c) return;
      noiseBurst(c, c.currentTime, 0.012, 0.055, 'highpass', 2600, 2600);
    },
