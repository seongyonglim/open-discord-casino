 ...
 *
 * 게임을 해서 깨는 과제는 기본이 1,000P 다 — 1P 씩 수천 번 돌려 긁어내면 과제가
 * "무엇을 해냈나"가 아니라 "얼마나 오래 눌렀나"의 기록이 된다.
 * 게임의 코인 단위가 달라 그 값이 맞지 않으면 과제 쪽에서 따로 정한다
 * (한탕주의자 pk-quads-plus 는 500 — 포커 플립의 최소 코인이 그것이다).
...
export function achievementsFor(userId: string | null): AchievementView[] {
  ...
  return listAchievements().map(a => {
    ...
    /* 많이 해낸 것이 위로 온다 — 달성자가 많은 순이다.
       한때 반대로(희귀한 것부터) 세웠다. 그러면 목록 위쪽이 아무도 못 깬 과제로 채워져
       처음 열어 본 사람에게는 "할 수 있는 것이 없는 목록"으로 읽힌다. 지금 순서는 위쪽이
       "남들이 해낸 것" = 해볼 만한 것이고, 어려운 것은 내려가면 나온다.

       달성률(=달성자/참가자)로 나눠 보지 않는다. 분모가 모두 같아서 순서가 달성자 수와
       똑같고, 나누기만 늘어난다.

       달성 여부는 순서에 넣지 않는다 — 카드 색이 살아 있어 구분은 그것으로 충분하다.

       ── 동률은 왜 따로 안 적나
       listAchievements 가 이미 sort_at → id 순으로 주고, JS 정렬은 안정 정렬이라
       (ES2019 부터 명세) 달성자 수가 같은 것들은 들어온 순서, 즉 게임별 묶음 순서를
       그대로 지킨다. 그래서 비교 함수 한 줄로 "달성자 많은 순 → 게임별 묶음 → id"가 된다.
       시즌 초에는 대부분이 0명이라 사실상 게임별 묶음으로 보이고, 시즌이 익으면서
       달성자 수가 자연히 주도권을 가져간다. 정렬을 손으로 다시 매길 일이 없다. */
    .sort((x, y) => y.unlockedBy - x.unlockedBy);
}
