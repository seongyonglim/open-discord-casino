...
/* 쇼다운을 보여주는 시간 (다음 핸드까지). 보드가 이미 다 깔린 리버 쇼다운 기준이다.
   ...
   한때 6초였다. 그때는 승자 판정 대기가 1.4초, 칩 이동이 0.9초여서 4초면 끝났다.
   연출을 늘렸으면 이 값도 같이 늘려야 한다 — 안 그러면 마지막 칩이 흡수되기 전에
   다음 판이 시작된다.
...
/* 폴드로 끝난 판.
   ...
   똑같이 적용된다. 배지 대기가 2.5초, 칩 이동이 1.8초, 흡수 뒤 여유가 1.75초라
   폴드 판도 쇼다운과 같은 길이를 쓴다(칩 수가 적어 시차만 짧다).
   그래서 SHOWDOWN_SEC과 같은 8초다 — 예전에는 "읽을 것이 승자 이름뿐"이라 짧게
   뒀는데, 지금은 읽는 시간이 아니라 연출 길이가 이 값을 정한다. */
export const FOLD_END_SEC = 8;
