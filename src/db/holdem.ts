...
/* 폴드로 끝난 판.
   ...
   여기에 WIN 배지가 들어왔다 — 칩보다 먼저 승자를 보여주는 순서가 폴드로 끝난 판에도
   똑같이 적용된다. 결과 지연 1.5초 · 배지 대기 2.5초 · 칩 이동 1.8초 · 흡수 뒤 1.75초는
   폴드 판에도 그대로 흐른다. 그래서 SHOWDOWN_SEC과 같은 값이다 — 예전에는 "읽을 것이
   승자 이름뿐"이라 짧게 뒀는데, 지금은 읽는 시간이 아니라 연출 길이가 이 값을 정한다. */
export const FOLD_END_SEC = 10;
/* 사이드 팟을 하나 더 보여주는 데 드는 시간.
   화면은 층마다 [WIN 배지 → 1.0초 대기 → 칩 이동 1.8초 → 시차]를 재생한다.
   3.5초쯤 걸리므로 4초를 준다. 더 끌면 판이 늘어지고, 더 줄이면 칩이 도착하기 전에
   다음 판이 시작돼 마지막 사이드 팟을 놓친다.
   (첫 층의 대기 2.5초는 SHOWDOWN_SEC 쪽에 이미 들어 있다.) */
export const SIDE_POT_STEP_SEC = 4;
