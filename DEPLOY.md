...
### 8-2. scale-to-zero 동작 확인 (요금에 직결)
...
### 8-3. 디스코드 3초 제한 확인 (중요)

디스코드 인터랙션은 **3초 안에** 응답해야 한다.
머신이 잠들어 있을 때 fly가 깨우는 시간이 이 안에 들어오는지 **실측해야 한다.**
...
- `fly.toml`의 `auto_stop_machines`를 `"suspend"`로 바꾼다.
  suspend는 stop보다 재개가 빠르고(커뮤니티 보고 100~250ms), 256MB는 2GiB 제한 안에 든다.
  단 **볼륨이 붙은 머신에서의 동작과 suspend 중 과금은 공식 문서에 없다 — 확인 불가.**
  바꾼 뒤 8-2와 8-3을 다시 돌려야 한다.
- 또는 `min_machines_running = 1`로 상시 가동한다. 확실하지만 요금이 $2.02/월로 고정된다.
