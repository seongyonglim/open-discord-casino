...
/* 사람 대신 두는 안전장치.
   내 차례를 그냥 두면 20초 뒤 자동 폴드되고, 그 다음부터 자리 비움으로 내려가
   판이 휙휙 지나간다. 그렇다고 매번 대신 두면 직접 눌러볼 수가 없다.
   그래서 "남은 시간이 이 값보다 적어지면" 그때만 대신 둔다 — 그 전까지는 사람 차례다.

   이 값이 6이면 화면에서는 "7초에 강제로 체크됐다"처럼 보인다(폴링이 1초 간격이라
   6초를 보는 순간 누른다). 실제 게임의 제한 시간은 20초이고 0초에 자동 체크된다 —
   이건 시뮬레이션만의 안전장치다.

   마지막 5초 연출(고리가 붉어지고 똑딱 소리가 나는 구간)을 보려면 이 값을 0으로 둬서
   아예 대신 두지 않게 한다: SIM_ME_GRACE=0 npx tsx scripts/sim-holdem.ts */
const ME_GRACE_SEC = Number(process.env.SIM_ME_GRACE ?? 6);
let mePlayedFor = 0;
